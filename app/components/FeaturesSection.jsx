...
export default function FeaturesSection() {
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10 sm:mt-16">
      <div className="bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-10 border border-slate-200 shadow-sm">
        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-center text-slate-900">
          Why Patients & Doctors Trust RxConnect
        </h2>

        <p className="text-center text-slate-600 mt-2 mb-8 text-sm sm:text-base max-w-2xl mx-auto">
          Combining multi-branch inventory speed with hospital-grade
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {features.map((item, index) => (
            <div
              key={index}
              className="border border-slate-200 rounded-2xl p-5 sm:p-6 bg-slate-50 hover:border-emerald-300 transition"
            >
              <div
              ...
              <h3 className="text-slate-900 font-semibold text-base sm:text-lg mt-4 sm:mt-5">
                {item.title}
              </h3>

              <p className="text-slate-600 mt-2 sm:mt-3 text-xs sm:text-sm leading-relaxed">
                {item.desc}
              </p>
            </div>
          ))}
